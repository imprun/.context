...
sequenceDiagram
    autonumber
    actor Operator
    participant UI as Agent 등록 폼
    participant API as imprun-server
    participant Agent as Agent (K8s Pod)
    participant K8s as Kubernetes

    Operator->>UI: "Agent 등록" 클릭
    UI->>UI: 등록 폼 표시
    Operator->>UI: Agent 이름 입력
    UI->>API: POST /api/v1/operator/agents (name, cluster_id)
    API-->>UI: Agent 생성 + Bootstrap Token
    UI-->>Operator: 토큰 표시 (1회만)

    Note over Operator: 토큰 복사

    Operator->>K8s: Agent 배포 (Helm/kubectl)
    Note over K8s: AGENT_TOKEN 환경변수 설정
...
stateDiagram-v2
    ...
    Active --> Inactive: Heartbeat 타임아웃 (5분)
    Inactive --> Active: Heartbeat 재개
    Active --> [*]: 삭제
    Pending --> [*]: 삭제
...
sequenceDiagram
    actor Operator
    participant List as Cluster 상세
    participant Form as Agent 등록 폼
    participant Token as 토큰 표시 모달
    participant API as imprun-server

    Operator->>List: Cluster 상세 페이지
    Operator->>List: "Agent 등록" 클릭
    List->>Form: 등록 폼 모달 표시

    Operator->>Form: Agent 이름 입력
    Form->>API: POST /api/v1/operator/agents
    API-->>Form: { agent_id, bootstrap_token }

    Form->>Token: 토큰 표시 모달
    Note over Token: ⚠️ 이 토큰은 다시 볼 수 없습니다!

    Operator->>Token: "복사" 클릭
    Token-->>Operator: 클립보드에 복사됨

    Operator->>Token: "완료" 클릭
    Token->>List: Cluster 상세로 돌아감
    Note over List: 새 Agent (Pending) 표시
```
